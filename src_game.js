// Robust Arcade Depth Prototype - replacement file
// If a fetch fails, this will fall back to inline level data and log helpful info.
// Paste this into src/game.js (replace existing) and commit on GitHub, then reload your Pages site.

console.log("Arcade Depth: starting script");
...
if (!canvas) {
  console.error("Canvas element with id='game' not found in DOM.");
} 
...
function makeTouchButtons(){
  ...
  if (!overlay) return;
  overlay.innerHTML = ''; // ensure single set
...
// Inline fallback level (used if fetch fails)
const INLINE_LEVEL = {
...
const player = {
  ...
  width:36, height:56,
  speed:320, accel:2200, drag:1200,
  jumpForce:520, maxFall:1400,
  grounded:true, facing:1, color:'#ffd559',
  shotCooldown:0
};
...
// helper to init world from level object
function initLevelFromObject(lvl) {
  ...
  console.log("Initialized level:", level.meta && level.meta.name);
}

// Attempt to fetch JSON level; fallback to inline
async function loadLevelWithFallback(url) {
  try {
    console.log("Trying to fetch level:", url);
    ...
    console.log("Level loaded from", url);
  } catch (err) {
    console.warn("Failed to load level JSON, falling back to inline data. Error:", err);
    initLevelFromObject(INLINE_LEVEL);
  }
}

// If level file exists under the same path, attempt to load; otherwise fallback
loadLevelWithFallback('src/levels/level1.json').catch(err => {
  console.error("loadLevelWithFallback error (should be handled above):", err);
  initLevelFromObject(INLINE_LEVEL);
});
...
function spawnBullet(x,y,z,dir,speed=520,owner='player'){
  world.bullets.push({x,y,z,vx:dir*speed,vy:0,vz:0,owner,ttl:3});
}
function spawnParticle(x,y,life=0.5,color='#fff'){
  world.particles.push({x,y,life,age:0,color,vy:-30+Math.random()*-60, vx:(Math.random()-0.5)*80});
}
function spawnExplosion(x,y,z){
  for (let i=0;i<12;i++){
    spawnParticle(x - camera.x + (Math.random()-0.5)*30, y + (Math.random()-0.5)*30, 0.8, '#ffcc66');
  }
}
...
function update(dt){
  ...
  for (const b of world.bullets){
    ...
    b.z += (b.vz || 0) * dt;
    b.ttl -= dt;
    try { checkBulletHit(b); } catch(err){ console.error("Bullet hit check error", err); }
  }
  ...
  debugEl && (debugEl.innerText = `X:${player.x.toFixed(0)} Z:${player.z.toFixed(1)} vx:${player.vx.toFixed(0)} grounded:${player.grounded}`);
}
...
function draw(){
  ...
  // sky
  ctx.fillStyle = '#87c5ff';
  ctx.fillRect(0,0,W,H);

  // parallax
  const layers = [
    { color:'#7fc3ff', speed:0.15, y:80 },
    { color:'#6ab0c9', speed:0.35, y:220 },
    { color:'#2b4b52', speed:0.65, y:360 }
  ];
  for (const L of layers){
    const ox = - (camera.x * L.speed) % (W*2);
    ctx.fillStyle = L.color;
    ctx.fillRect(ox - W, L.y, W*3, H - L.y);
  }
  ...
  function drawEntity(x,y,z,w,h,color,extra){
  ...
  drawEntity(player.x, player.y, player.z, player.width, player.height, player.color, (sx,sy,scale)=>{
    ctx.fillStyle = '#000';
    ctx.fillRect(Math.round(sx + Math.max(4,6*scale)), Math.round(sy + Math.max(6,8*scale)), Math.max(2,Math.round(4*scale)), Math.max(2,Math.round(4*scale)));
  });

  for (const e of world.enemies){
    if (e.dead) { drawEntity(e.x, e.y, e.z, 28, 36, '#8b5a5a'); continue; }
    const color = e.type === 'ranged' ? '#ff6e6e' : '#6ee6ff';
    drawEntity(e.x, e.y, e.z, 28, 36, color);
  }
...
function loop(now){
  ...
  last = now;
  try { update(dt); draw(); } catch(err) { console.error("Error in update/draw:", err); }
...
console.log("Arcade Depth: script loaded - if you don't see the level, open DevTools Console for errors.");
